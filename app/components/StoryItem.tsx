...
function formatDate(timestamp: number, format: 'relative' | 'short' | 'iso'): string {
...
export default function StoryItem({ story, viewMode, isSingleView = false }: StoryItemProps) {
  ...
  const id = 'objectID' in story ? story.objectID : story.id
  const title = story.title || 'Untitled'
  const url = story.url || (typeof window !== 'undefined' ? `${window.location.origin}/item/${id}` : `/item/${id}`)
  const points = 'points' in story ? story.points : story.score
  const author = 'author' in story ? story.author : story.by
  const timestamp = 'created_at' in story ? new Date(story.created_at).getTime() / 1000 : story.time
  const comments = 'num_comments' in story ? story.num_comments : story.descendants
  const text = 'text' in story ? story.text : undefined

  const domain = story.url ? new URL(story.url).hostname.replace(/^www\./, '') : null
  const formattedTime = formatDate(timestamp, timeFormat)
  ...
  const handleSetAsTask = () => {
    addTask({
      ...
      source: 'hackernews',
      projectId: 'default',
      notificationsEnabled: false // Add this line
    })
  }
          ...
          <div className="flex flex-col gap-2 mb-2">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between">
                ...
                {isTaskItem && (
                  <Badge variant="default" className="text-xs bg-green-500 text-white ml-2">
                    Task
                  </Badge>
                )}
              </div>
